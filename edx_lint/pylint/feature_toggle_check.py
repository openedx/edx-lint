...
class FeatureToggleChecker(BaseChecker):
    ...
    _WAFFLE_TOGGLE_CLASSES = (
        "WaffleFlag",
        "WaffleSwitch",
        "CourseWaffleFlag",
    )
    _ILLEGAL_WAFFLE_FUNCTIONS = [
        "flag_is_active",
        "switch_is_active",
    ]
    ...
    def check_waffle_class_annotated(self, node):
        ...
        if not self._lines.is_line_annotated(node.lineno - 1):
            ...
            self.add_message(
                self.TOGGLE_NOT_ANNOTATED_MESSAGE_ID, args=(feature_toggle_name,), node=node,
            )
    ...
    def check_configuration_model_annotated(self, node):
        ...
        if not self._lines.is_line_annotated(node.lineno - 1):
            ...
            self.add_message(
                self.TOGGLE_NOT_ANNOTATED_MESSAGE_ID, args=(config_model_subclass_name,), node=node,
            )
